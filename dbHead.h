...
int initRelationName(struct dbSysHead *head, long fid, char *name);//初始化关系名字
int initConstructor(struct dbSysHead *head, long fid, char *name);//初始化建立者
int changeAttributeNum(struct dbSysHead *head, long fid, int num);//更改属性个数
int changeRecordLength(struct dbSysHead *head, long fid, int length);//更改记录长度
int changeRecordNum(struct dbSysHead *head, long fid, int num);//更改记录总数
int initAttribute(struct dbSysHead *head, long fid, char *name, int type, int length, int deviation);//初始化一个属性表
int deleteAttribute(struct dbSysHead *head, long fid, char *name);//删除一个属性表
