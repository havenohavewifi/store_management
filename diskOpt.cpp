...
* @param [in] name  : char* 关系名
* @return int
*
* @author mengxi
* @date 2015/11/4
**/
int initRelationName(struct dbSysHead *head, long fid, char *name)
{
	int n;
	...
	strcpy((head->desc).fileDesc[n].relationDef.relationName, name);
	return 0;
}
...
* @param [in] head  : struct dbSysHead *
* @param [in] fid   : long  文件标识
* @param [in] name  : char* 建立者姓名
* @return int
*
* @author mengxi
* @date 2015/11/4
**/
int initConstructor(struct dbSysHead *head, long fid, char *name)
{
	int n;
	...
	strcpy((head->desc).fileDesc[n].relationDef.constructor, name);
	return 0;
}
...
* @param [in] head  : struct dbSysHead *
* @param [in] fid   : long  文件标识
* @param [in] num   : int   属性个数
* @return int
*
* @author mengxi
* @date 2015/11/4
**/
int changeAttributeNum(struct dbSysHead *head, long fid, int num)
{
	int n;
	...
	(head->desc).fileDesc[n].relationDef.attributeNum = num;
	return 0;
}
...
* @param [in] head  : struct dbSysHead *
* @param [in] fid   : long  文件标识
* @param [in] length: int 记录长度
* @return int
*
* @author mengxi
* @date 2015/11/4
**/
int changeRecordLength(struct dbSysHead *head, long fid, int length)
{
	int n;
	...
	(head->desc).fileDesc[n].relationDef.recordLength = length;
	return 0;
}
...
int changeRecordNum(struct dbSysHead *head, long fid, int num)
{
	...
	(head->desc).fileDesc[n].relationDef.recordNum = num;
	return 0;
}
...
* @param [in] deviation： int 记录内偏移
* @return int
*
* @author mengxi
* @date 2015/11/4
**/
int initAttribute(struct dbSysHead *head, long fid, char *name, int type, int length, int deviation)
{
	int n;
	int pos;
	...
	pos = (head->desc).fileDesc[n].relationDef.attributeNum;
	strcpy((head->desc).fileDesc[n].relationDef.attribute[pos].attributeName, name);
	(head->desc).fileDesc[n].relationDef.attribute[pos].length = length;
	(head->desc).fileDesc[n].relationDef.attribute[pos].type = type;
	(head->desc).fileDesc[n].relationDef.attribute[pos].recordDeviation = deviation;
